...
| **Non-Aviate** | Kill Bill account **custom fields** (`taxExempt`, `customerUsageType`) | Per invoice-item **custom fields** (`taxCode`) |

Resolution order:

1. **Plugin properties** (primary — Aviate plugin will pass these automatically)
...
| Property | Purpose |
|----------|---------|
| `customerUsageType` | Entity use code |
...
| `taxCode_<invoiceItemId>` | Per-line product tax code |
...
* **Plugin properties** — documented names in `InvoicePluginPropertyNames`; properties override custom fields.
